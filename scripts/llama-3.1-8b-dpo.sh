...
# N iterations of dpo
for i in {1..1}; do
    echo "starting DPO iteration $i of 1"

    cd /workspace/sleeperer-agents/liars
    # generate set of training data
    python data.py --split train --prefix $1

    # round of training
    cd /workspace
    read -r -d '' training_commands <<EOF
openrlhf.cli.train_dpo \
    --save_path /workspace/models/llama-3.1-8b-dpo-oversample-incorrect-$1 \
    --eval_steps 1000 \
    --max_ckpt_num 1 \
...
EOF
    ...
    echo "finished DPO iteration $i of 1"
done

# upload model
# cd /workspace/sleeperer-agents/tools
# python upload_model.py --model llama-3.1-8b-dpo-$1 --name llama-3.1-8b-dpo-$1-2604